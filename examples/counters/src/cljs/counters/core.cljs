...
;;
;; And note how we use (rdc/with-ctx) to prefix the actions
...
(def counter-reducer  
  (rdc/composite-reducer
    {:1 (rdc/reducer 
          {:counter 10} 
          (rdc/with-ctx :counter1 counter-actions))
     :2 (rdc/reducer 
          {:counter 20} 
          (rdc/with-ctx :counter2 counter-actions))}))

(def app-reducer
  (rdc/composite-reducer
    {:counters counter-reducer}))
